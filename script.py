...
def get_info(soup):
    ...
    links = job_link(soup)
    results_dataframe = pd.DataFrame(list(zip(titles, companies, locations, links)), columns = columns)
...
def main():
    job, city, state, email = get_args()
    soup = set_up(job, city, state)
    results_dataframe = get_info(soup)
